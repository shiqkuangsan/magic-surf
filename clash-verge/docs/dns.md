...
## 1.3 企业 VPN（EasyConnect / AnyConnect 等）+ TUN 共存方案

企业 VPN 客户端常推送两类东西到操作系统：

1. **虚拟网卡 + 内网路由**（如 `172.16.0.0/12` 走 VPN 隧道）
2. **内网 DNS server IP**（用于解析 `*.corp-domain.com` 之类只在内网生效的域名）

TUN 模式下 mihomo 接管全机 DNS，会把发往内网 DNS server 的查询包**也吃掉**——结果是公网 DoH 解析不到内网域名，VPN 建好后内网服务全部访问失败。

正确的共存配方需要三件事配合：

| 配置位 | 字段 | 作用 |
|--------|------|------|
| `shared/Merge.yaml` 或 Verge GUI DNS 设置 | `dns.nameserver-policy: "+.<corp-domain>": "<内网 DNS IP>"` | 让 mihomo 把内网域名查询定向到内网 DNS server，不走默认公网 DoH |
| `shared/Merge.yaml` 或 Verge GUI DNS 设置 | `dns.fake-ip-filter` 加 `+.<corp-domain>` | 让 mihomo 对内网域名返回真实 IP，不返 fake-ip（部分企业 app 不接受 fake-ip） |
| `shared/Merge.yaml` | `tun.route-exclude-address: <内网 DNS IP>/32` | mihomo 自身发出的去往内网 DNS 的 53 端口包**绕过 TUN**，由 VPN 客户端虚拟网卡路由进隧道 |

⚠️ **VPN 入口域名（如 `vpn.<corp>.com`）必须额外列出走公网 DNS**——否则会与 `+.<corp>.com → <内网 DNS>` 死锁（VPN 没建好时内网 DNS 不可达，永远连不上 VPN）：

```yaml
dns:
  nameserver-policy:
    "vpn.example-corp.com":      # VPN 入口本身走公网解析
      - 223.5.5.5
      - 119.29.29.29
    "+.example-corp.com":        # 其它内网域名走 VPN 内网 DNS
      - 10.x.x.x
```

确定 `<内网 DNS IP>` 的方法：连上 VPN 后执行 `scutil --dns | grep "nameserver\[0\]" | sort -u`，新增的非系统默认 DNS 即是 VPN 推送的。

## 2. 两条等价路径：`Merge.yaml` vs Verge GUI「DNS 设置」

Verge Rev 提供两个写 DNS 的入口，**最终都注入 mihomo runtime config 的 `dns:` 字段**，二者只用其一即可：

| 路径 | 落地文件 | 适用场景 |
|------|---------|---------|
| `shared/Merge.yaml` 的 `dns:` 块（YAML deep-merge） | 所在仓库版本控制 | fork 者直接用本仓库 baseline / 想把 DNS 配置纳入 Git 同步多机 |
| Verge GUI →「DNS 设置」面板 | Verge Application Support 下的 `dns_config.yaml`（Verge 自管，不在本仓库） | 单机使用 / 想用 GUI 即时调整不重启 Verge |

⚠️ **不要同时启用两条**：Verge GUI「DNS 设置 → 启用 DNS 设置」开关打开时，GUI 维护的 `dns_config.yaml` 会**完全覆盖** `Merge.yaml` 里的 `dns:` 块（不是 deep-merge，是替换）。如果想用 `Merge.yaml` 的 baseline，需把 GUI 开关关闭。
...
典型软路由 + Mac 双层架构下：iStoreOS / OpenWRT 上跑 dnsmasq，Mac 系统代理模式下系统 DNS 指向软路由 LAN IP，TUN 模式下 mihomo `dns:` 块的 default-nameserver 也可改指向 `192.168.x.x:53`（软路由 IP）。
...
| 内网域名（`*.lan` / `*.example-corp.com`）走代理出去 | 加进 `fake-ip-filter`，必要时配合 `nameserver-policy` 精确指向内网 DNS |
| TUN 模式 + 企业 VPN（EasyConnect 等）连入后内网服务访问不通 | 见 [§ 1.3](#13-企业-vpneasyconnect--anyconnect-等--tun-共存方案)：检查 `nameserver-policy` 是否指向 VPN 推送的内网 DNS，且 `route-exclude-address` 是否包含该 DNS IP |
| VPN 入口域名（`vpn.example-corp.com`）解析失败 / 永远连不上 | 入口域名被 `nameserver-policy` 误指向内网 DNS（鸡生蛋）；显式给入口域名单独配置公网 DNS policy |
...
- ACL4SSR 模板默认不带 `dns:` 块——`Merge.yaml` 的 `dns:` 块或 Verge GUI「DNS 设置」面板二选一，是 fork 后的**主要 DNS 真值源**
